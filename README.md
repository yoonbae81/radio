...
맥미니처럼 항상 켜 두는 환경에서 "정해진 시간에 녹음"이 목적이라면 Python 웹앱 전체 대신 `launchd + ffmpeg` 기반의 더 단순한 macOS 전용 구성을 쓸 수 있습니다.

- 녹음만 필요할 때
- 녹음 후 정적 `feed.xml`까지 만들어 팟캐스트 앱에 연결하고 싶을 때

설정과 설치 방법은 [scripts/macos/README.md](scripts/macos/README.md) 에 정리해 두었습니다.
